...
{
    public class OverTimeScoreBoardServ
    {
        public IndexFilters indexFilters { get; set; }
        public int totalRoundCount { get; set; }
        public OverTimeScoreBoardServ(int totalRoundCount)
        {
            indexFilters = new IndexFilters();
            this.totalRoundCount = totalRoundCount;
        }
        public KillTableModel SetOverTimeScoreBoard(ServerDataBase serverDataBase)
        {
        ...
        public KillTableModel SetOverTimeScoreBoard(ServerDataBase serverDataBase, EconomyFilters_ScoreBoard economyFilters_ScoreBoard)
        {
            for (int i = 30; i < totalRoundCount; i++)
            {
        ...
        public KillTableModel SetOverTimeScoreBoard(float timestamp, string Phase, ServerDataBase serverDataBase, EconomyFilters_ScoreBoard economyFilters_ScoreBoard, TimerFunction timerFunction)
        {
            for (int i = 30; i < totalRoundCount; i++)
            {
                var RoundEconomyFilter = indexFilters.GetRoundEconomyStateFilter(i);
